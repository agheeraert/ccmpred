...
class LogLossFactorized(nn.Module):
	...
	'''
	def __init__(self, L, q=21, gpu=False, lambda_h=0.01, lambda_J=0.001):
		super(LogLossFactorized, self).__init__()
		self.L = L
		self.q = q
		
		self.K = nn.Parameter(torch.FloatTensor(self.q, self.q).cuda())
		self.H = nn.Parameter(torch.FloatTensor(self.q, self.L).cuda())
		self.J = nn.Parameter(torch.FloatTensor(self.L, self.L).cuda())
