...
if __name__=='__main__':
    ...
    for i, aa in enumerate(msa[b]):
        s_i.append(aa_to_idx[aa]*q + aa_to_idx[msa[b][r]])
    ...
    for i in range(0,q):
        for j, aa in enumerate(msa[b]):
            all_aa_si[i,j] = aa_to_idx[aa]*q + i
    all_aa_si = Variable(all_aa_si)
    
    #Computing nominator
    J_rij = J(s_i).resize(L, L, L).permute(1, 2, 0)[:,r,:]
    nominator = torch.exp(H(s_r)[0,r] + (J_rij*mask).sum())

    #Computing denominator
    J_rili = J(all_aa_si).resize(q,L,L,L).permute(0, 2, 3, 1)
    J_ili = J_rili[:,:,r,:]
